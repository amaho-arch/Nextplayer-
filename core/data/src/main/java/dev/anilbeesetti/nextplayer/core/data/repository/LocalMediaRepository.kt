...
class LocalMediaRepository @Inject constructor(
    ...
    override suspend fun updateIntroTimestamps(uri: String, introStartMs: Long, introEndMs: Long) {
        val stateEntity = mediumStateDao.get(uri) ?: MediumStateEntity(uriString = uri)
        mediumStateDao.upsert(
            mediumState = stateEntity.copy(
    ...
    override suspend fun getIntroTimestamps(uri: String): Pair<Long, Long>? {
        return mediumStateDao.get(uri)?.let { entity ->
            Pair(entity.introStartMs, entity.introEndMs)
        }
    }

    override suspend fun updateFolderIntroTimestamps(folderPath: String, introStartMs: Long, introEndMs: Long) {
        mediumStateDao.upsertFolderIntro(FolderIntroEntity(folderPath, introStartMs, introEndMs))
    }

    override suspend fun getFolderIntroTimestamps(folderPath: String): Pair<Long, Long>? {
        return mediumStateDao.getFolderIntro(folderPath)?.let { entity ->
            Pair(entity.introStartMs, entity.introEndMs)
        }
    }

    override suspend fun clearFolderIntroTimestamps(folderPath: String) {
        mediumStateDao.deleteFolderIntro(folderPath)
    }
}
