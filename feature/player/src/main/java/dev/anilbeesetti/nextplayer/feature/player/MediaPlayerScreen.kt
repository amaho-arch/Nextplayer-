...
fun MediaPlayerScreen(
    ...
    val longPressSpeedKey = (playerPreferences.longPressControlsSpeed * 10).toInt()
    LaunchedEffect(longPressSpeedKey) {
        tapGestureState.updateLongPressSpeed(playerPreferences.longPressControlsSpeed)
    }
    ...
    CompositionLocalProvider(LocalControlsVisibilityState provides controlsVisibilityState) {
        Box {
            Box(
                ...
                AnimatedVisibility(
                    ...
                    Surface(
                        ...
                        Row(
                            ...
                            Icon(
                                imageVector = NextIcons.FastForward,
                ...
                val showSkipIntroOverlay = remember { mutableStateOf(false) }
                LaunchedEffect(mediaPresentationState.hasIntro, mediaPresentationState.position, mediaPresentationState.introStartMs, mediaPresentationState.introEndMs) {
                    val hasIntro = mediaPresentationState.hasIntro
                    val position = mediaPresentationState.position
                    val introStart = mediaPresentationState.introStartMs
                    val introEnd = mediaPresentationState.introEndMs
                    val shouldShow = hasIntro && position >= introStart && position < introEnd
                    if (shouldShow && !showSkipIntroOverlay.value) {
                ...
                AnimatedVisibility(
                    modifier = Modifier
                        .padding(top = 80.dp)
                        .align(Alignment.TopCenter),
                    visible = showSkipIntroOverlay.value,
                    enter = fadeIn(),
                    exit = fadeOut(),
                ) {
                    Surface(
                        ...
                        PlayerButton(onClick = { player.seekTo(mediaPresentationState.introEndMs) }) {
                            Row(
                                modifier = Modifier.padding(
                                    horizontal = 16.dp,
                                    vertical = 10.dp,
                                ...
                                Text(
                                    text = stringResource(coreUiR.string.skip_intro),
                    ...
                    PlayerControlsView(
                        topView = {
                            AnimatedVisibility(
                                ...
                                ControlsTopView(
                                    ...
                                    onMarkIntroClick = {
                                        controlsVisibilityState.hideControls()
                                        overlayView = OverlayView.MARK_INTRO
                                    },
                                    showMarkIntro = true,
                                )
                            }
                        ...
                        bottomView = {
                            AnimatedVisibility(
                                ...
                                val showSkipIntro = mediaPresentationState.hasIntro &&
                                    mediaPresentationState.position >= mediaPresentationState.introStartMs &&
                                    mediaPresentationState.position < mediaPresentationState.introEndMs
                                ControlsBottomView(
                                    player = player,
                                    ...
                                    onSkipIntroClick = {
                                        player.seekTo(mediaPresentationState.introEndMs)
                                    },
            ...
            val folderPath = player.currentMediaItem?.mediaId?.let { mediaId ->
                Uri.parse(mediaId).path?.substringBeforeLast("/")?.substringBeforeLast("/")
            }
