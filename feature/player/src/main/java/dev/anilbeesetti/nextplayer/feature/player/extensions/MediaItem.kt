...
private const val MEDIA_METADATA_INTRO_START_KEY = "media_metadata_intro_start"
private const val MEDIA_METADATA_INTRO_END_KEY = "media_metadata_intro_end"
...
private fun Bundle.setExtras(
    ...
    introStartMs: Long? = null,
    introEndMs: Long? = null,
) = apply {
    ...
    introStartMs?.let { putLong(MEDIA_METADATA_INTRO_START_KEY, it) }
    introEndMs?.let { putLong(MEDIA_METADATA_INTRO_END_KEY, it) }
}
...
val MediaMetadata.introStartMs: Long?
    get() = extras?.run {
        getLong(MEDIA_METADATA_INTRO_START_KEY)
            .takeIf { containsKey(MEDIA_METADATA_INTRO_START_KEY) }
    }

val MediaMetadata.introEndMs: Long?
    get() = extras?.run {
        getLong(MEDIA_METADATA_INTRO_END_KEY)
            .takeIf { containsKey(MEDIA_METADATA_INTRO_END_KEY) }
    }
...
fun MediaItem.copy(
    ...
    introStartMs: Long? = this.mediaMetadata.introStartMs,
    introEndMs: Long? = this.mediaMetadata.introEndMs,
