...
class PlayerViewModel @Inject constructor(
    ...
    fun updateIntroTimestamps(uri: String, introStartMs: Long, introEndMs: Long) {
        viewModelScope.launch {
            mediaRepository.updateIntroTimestamps(uri, introStartMs, introEndMs)
        }
    }

    suspend fun getIntroTimestamps(uri: String): Pair<Long, Long>? {
        return mediaRepository.getIntroTimestamps(uri)
    }

    fun updateFolderIntroTimestamps(folderPath: String, introStartMs: Long, introEndMs: Long) {
        viewModelScope.launch {
            mediaRepository.updateFolderIntroTimestamps(folderPath, introStartMs, introEndMs)
        }
    }

    suspend fun getFolderIntroTimestamps(folderPath: String): Pair<Long, Long>? {
        return mediaRepository.getFolderIntroTimestamps(folderPath)
    }

    fun clearFolderIntroTimestamps(folderPath: String) {
        viewModelScope.launch {
            mediaRepository.clearFolderIntroTimestamps(folderPath)
        }
    }

    fun updateLongPressSpeed(speed: Float) {
        viewModelScope.launch {
            preferencesRepository.updatePlayerPreferences { it.copy(longPressControlsSpeed = speed) }
        }
    }
}
