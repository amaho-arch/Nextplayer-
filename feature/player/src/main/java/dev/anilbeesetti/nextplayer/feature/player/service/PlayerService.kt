...
import dev.anilbeesetti.nextplayer.feature.player.extensions.introEndMs
import dev.anilbeesetti.nextplayer.feature.player.extensions.introStartMs
    ...
    private suspend fun updatedMediaItemsWithMetadata(
        ...
        mediaItems.map { mediaItem ->
            async {
                ...
                val introTimestamps = mediaRepository.getIntroTimestamps(uri = mediaItem.mediaId)
                val folderPath = uri.path?.substringBeforeLast("/")?.substringBeforeLast("/")
                val folderIntroTimestamps = folderPath?.let { mediaRepository.getFolderIntroTimestamps(it) }
                val introStart = mediaItem.mediaMetadata.introStartMs ?: introTimestamps?.first ?: folderIntroTimestamps?.first ?: 0L
                val introEnd = mediaItem.mediaMetadata.introEndMs ?: introTimestamps?.second ?: folderIntroTimestamps?.second ?: 0L
